...
export class SavingsCalculatorComponent implements OnInit {
  ...
  constructor(private _route:ActivatedRoute,private _firebase:FirebaseServiceService) {
    ...
    this._route.params.subscribe((params)=>this.activePill=params.type);
  }

  ngOnInit() {
  }
  ngAfterViewInit(){
    if(this.activePill=='savingsGoal'){
      this.submitForm(this.savingsForm);
    }
    else if(this.activePill=='savingsEstimator'){
      this.submitSavingsEstimator(this.savingsEstimator)
    }
    else{
      this.submitGoalTime(this.timeEstimator)
    }
